...
update_tabs() {
  ...
  for mon in $mon_names; do
    local mon_id ws_id state_file last_file was_open mon_tabs mon_last count

    # Get the numerical ID (0, 1, 2...) and active workspace for this monitor
    mon_id=$(printf '%s' "$monitors_json" | jq -r ".[] | select(.name == \"$mon\") | .id" 2>/dev/null)
    ws_id=$(printf '%s' "$monitors_json" | jq -r ".[] | select(.name == \"$mon\") | .activeWorkspace.id" 2>/dev/null)
    [ -z "$ws_id" ] || [ -z "$mon_id" ] && continue

    # Track states per monitor ID
    state_file="${XDG_RUNTIME_DIR:-/tmp}/hyprland-tabs-open-$mon_id"
    last_file="${XDG_RUNTIME_DIR:-/tmp}/hyprland-tabs-last-$mon_id"
    ...
    # Added -c to jq to output as a compact, single-line string
    mon_tabs=$(
    ...
    # Only update Eww if the tabs for this specific monitor have actually changed
    if [ "$mon_tabs" != "$mon_last" ]; then
      printf '%s' "$mon_tabs" >"$last_file"
      eww update "windows_$mon_id=$mon_tabs" 2>/dev/null || true
    fi

    # Open or close the specific bar for this monitor
    if [ "$count" -lt 2 ]; then
      if [ "$was_open" -eq 1 ]; then
        eww close "tabbar_$mon_id" 2>/dev/null || true
        echo 0 >"$state_file"
      fi
    else
      if [ "$was_open" -eq 0 ]; then
        eww open "tabbar_$mon_id" 2>/dev/null || true
        echo 1 >"$state_file"
      fi
    fi
  done
